...
The phone scopes conversation data and sync checkpoints to its authenticated workspace in browser localStorage. A fresh launch verifies the session before showing cached chats; disconnecting or changing workspace clears visible chats and drafts. Storage failures remain visible, and browser storage limits may be lower than the server's 20 MB workspace limit. Use a separate browser profile or clear site data to remove stored copies on a shared device. Provider credentials never leave the execution host.
