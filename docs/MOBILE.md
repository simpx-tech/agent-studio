...
The phone shell follows both the height and vertical offset of the visible viewport while the keyboard is open. After dismissal it clears stale offsets and restores the home-indicator inset. Browser tabs use the CSS dynamic viewport; installed apps use the full window/large viewport because [Safari can underreport installed-app heights](https://bugs.webkit.org/show_bug.cgi?id=254868). The main screen, sidebar, backdrop, and modal overlay share that height, with safe-area padding inside it. The keyboard does not need the bottom inset. Focus transitions are checked through their animation, and pinch zoom stays under browser control. Message updates scroll only the conversation pane. These choices account for the distinction between the [layout and visual viewports](https://developer.mozilla.org/en-US/docs/Web/API/VisualViewport).
