...
The phone shell follows both the height and vertical offset of the visible viewport while the keyboard is open. After dismissal it clears stale offsets and restores the home-indicator inset. The document, body, main screen, sidebar, backdrop, and modal overlay use the dynamic viewport height at rest. Do not enlarge the app to the maximum of `vh`, `innerHeight`, and `clientHeight`: [iOS 26 reports include fullscreen metrics larger than the drawable page](https://bugs.webkit.org/show_bug.cgi?id=301994), which can clip the footer. The installed app uses an opaque black status bar, with web content below it, instead of requesting the translucent status-bar layout. Safe-area padding stays inside the app's height; the top inset accounts for keyboard panning, and the keyboard needs no bottom home-indicator inset. Focus transitions are checked through their animation, and pinch zoom stays under browser control. Message updates scroll only the conversation pane. These choices account for the distinction between the [layout and visual viewports](https://developer.mozilla.org/en-US/docs/Web/API/VisualViewport).
