...
- **A streaming reply**, on every run including CI: 120 reported events, 40 of them reasoning, must drive fewer than twelve whole-workspace saves and still save at least once.
- **A waiting reply**, on every run including CI: a reply that reported its first progress and then waits leaves its polls asking for the revision alone, with no `v1/state` in either direction and no save.
...
| Streaming 120 reported events | 150 ms | no task over 50 ms | saves that copy the workspace, or one save per event |
