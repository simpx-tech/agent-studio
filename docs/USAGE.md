...
Every chat has three compact progress bars directly below the message input: **Context**, **5-hour**, and **Weekly**. They replace the former Tools enabled caption. The context bar shows the latest response's provider-reported context count. Before a measurement, or when a reply supplies no usable request-level count, it says **Not measured yet**. Unknown bars use a striped track and omit a numeric progress value, preserving the distinction from a measured zero.

Click any bar to open reset times, budgets, and guidance in a panel above the input. The panel leaves the message box and Send button usable; close it with Escape, its Close button, or the same bar. Its title and Checked text align vertically with Close. There are no gauge or refresh buttons. **Fable weekly** appears in this expansion only for a Claude Fable selection, including a CLI default that resolves to Fable. The three primary groups remain unchanged. Hover the account line inside the input for the selected provider's tool-access policy; the message field's hover retains its keyboard shortcuts.
