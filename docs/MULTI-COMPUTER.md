...
The sidebar has **Active** and **History** tabs with conversation counts, each grouped by computer and folder. Use **Move to history** in the chat toolbar to archive an active conversation. To continue a History conversation, send a message; it automatically returns to Active. There is no separate restore button. Archiving preserves messages and settings and syncs across paired computers. Conversations without a project folder appear under **Standalone**, with a chat-bubble icon in the sidebar and chat toolbar. Hover for an explanation, or click **+** beside Standalone to start another conversation without a project folder on the same computer.
