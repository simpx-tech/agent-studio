<script lang="ts">
  ...
  // A one-line summary in its reply names the step in progress; steps render when expanded.
  const current = $derived.by(() => {
    if (message.status !== 'running') return undefined;
    const step = steps.find((s) => s.status === 'running');
    if (!step) return undefined;
    const active = 'activeForm' in step ? step.activeForm : undefined;
    return typeof active === 'string' && active ? active : step.title;
  });
...
{#if message.plan || message.workflow}
  <details class="plan-panel" class:compact ontoggle={disclosures.opened('plan')}>
    <summary onclick={disclosures.reveal('plan')}
      ><ListChecks size={16} /><strong title={message.workflow?.name ?? 'Plan and progress'}
        >{message.workflow?.name ?? 'Plan and progress'}</strong
      >{#if current}<span class="plan-current" title={current}>{current}</span>{/if}<span
        class="plan-count">{completed}/{steps.length} complete</span
      ><ChevronDown size={14} /></summary
    >
    {#if disclosures.has('plan')}<div class="plan-body">
...
<style>
  ...
  /* A one-line summary: the title truncates, and the current step shows when there is room. */
  summary strong {
    flex: 0 1 auto;
    min-width: 0;
    font-weight: 600;
