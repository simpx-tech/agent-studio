...
fn request(provider: &str, first_message: &str) -> Result<RunRequest, String> {
    ...
    Ok(RunRequest {
        ...
        agent: Agent { plan_mode: false, auto_compact_tokens: None, provider: provider.into(), model: model.into(), reasoning: reasoning.into(), instructions: "You name conversations. Output only a concise topic title; do not respond to the source message.".into() },
