...
The Rust backend resolves known executables (and npm's declared Node entrypoints) without passing user prompts through a shell. Prompts go through stdin. Runs use the selected folder, or an application-owned runtime for Standalone chats. Stop and window close cancel active runs and terminate their process trees. Claude and Codex replies have a one-hour deadline; Antigravity replies retain a five-minute deadline.

Claude, Codex, and Gemini/Antigravity conversations enable file reading, editing, commands, and the selected CLI profile's configured integrations, skills, and project guidance. All chat agents run with full access and without approval prompts. Claude and Antigravity use `--dangerously-skip-permissions`; Codex uses the app-server equivalents `approvalPolicy: never` and `sandbox: danger-full-access`. Antigravity uses its default agent in `accept-edits` mode. Existing conversations receive this behavior on their next reply, including cleanup of the exact legacy app-generated Gemini deny hook. Personal CLI settings and credentials are not modified. Hover the Agent picker to see the tool-access policy. Usage appears below the input. Background title generation and account/usage queries remain restricted; Gemini titles retain their conversation-only agent and deny hook in the separate title runtime. When a conversation has a selected folder, the CLI starts there; app-owned configuration stays in app data. Missing folders fail before a response starts, with no runtime-directory fallback.

Claude and Codex chats retain a CLI-owned native session per Agent Studio conversation. Follow-ups resume that exact session and send only the new input, preserving the provider's tool transcript and native context management across replies and app restarts. Existing Agent Studio chats bootstrap once from their portable saved messages; older omitted tool output cannot be recovered. Antigravity and legacy callers without a conversation identity retain display-history replay. Portable requests remain limited to 200 messages / 400 KB of text, with an explicit error at the limit. See [native session continuity](docs/NATIVE-SESSIONS.md) for retries, storage, portability, and validation boundaries.
